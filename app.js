...
// --- CENTRALIZED TASK SYNC & PERSISTENCE (POSTGRES API) ---
...
function mapRecordToTask(r) {
  return {
    ...
    proofReq: r.proofReq || r.proof_req || ''
  };
}
      ...
      .then(([records, subRecords]) => {
        ...
        if (!hasChanges) {
          for (let i = 0; i < newTasks.length; i++) {
            if (newTasks[i].id !== STATE.tasks[i].id || newTasks[i].completedCount !== STATE.tasks[i].completedCount) {
              hasChanges = true;
              break;
            }
          }
  ...
  function handleCreateTaskSubmit(event) {
    ...
    executeRealTransaction(
      ...
      async (txHash, receipt) => {
        ...
        apiCreateTask(newTask).catch(err => {
          console.error('PostgreSQL task creation failed:', err);
        });
    ...
    executeRealTransaction(
      ...
      async (txHash, receipt) => {
        ...
        if (task) {
          ...
          await apiUpdateTask(task.id, { completedCount: task.completedCount });
        }
